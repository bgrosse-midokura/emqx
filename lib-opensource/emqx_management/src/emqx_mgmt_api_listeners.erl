...
restart(#{node := Node, identifier := Identifier}, _Params) ->
    case emqx_mgmt:restart_listener(Node, emqx_listeners:find_by_id(Identifier)) of
...
restart(#{identifier := Identifier}, _Params) ->
    Results = [emqx_mgmt:restart_listener(Node, emqx_listeners:find_by_id(Identifier)) || {Node, _Info} <- emqx_mgmt:list_nodes()],
